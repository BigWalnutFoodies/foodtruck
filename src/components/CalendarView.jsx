        ...
        <div style={styles.card}>
          ...
          <div style={styles.grid}>
            {cells.map((date, i) => {
              ...
              return (
                <div key={i} style={{ ...styles.cell, ...styles['cell_' + status], ...(isToday ? styles.cellToday : {}) }}>
                  <span style={{ ...styles.dateNum, ...(status === 'past' ? { color: '#bbb' } : {}) }}>{date.getDate()}</span>
                  {status === 'available' && <span style={dot('#1a8a4a')} />}
                  {status === 'pending'   && <span style={dot('#F5A800')} />}
          ...
          <div style={styles.legend}>
            {[{ color: '#1a8a4a', label: 'Available' }, { color: '#F5A800', label: 'Pending' }, { color: '#C41230', label: 'Booked' }, { color: '#ccc', label: 'Past' }].map(l => (
...
const styles = {
  ...
  cell_pending:   { background: '#fff8e6', border: '1px solid #ffe0a0' },
