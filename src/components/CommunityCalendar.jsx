...
function formatDate(iso) {
  // iso = 'YYYY-MM-DD' — parse as local date to avoid UTC offset shifting the day
...
export default function CommunityCalendar() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('applications')
      .select('requested_date, business_name, cuisine, instagram, menu_link')
      .eq('status', 'approved')
    ...
    <section style={s.section}>
      <div style={s.container}>
        <div style={s.pageHeader}>
          <h1 style={s.h1}>Community Calendar</h1>
          ...
          <div style={s.grid}>
            {events.map((ev, i) => (
              <div key={i} style={s.card}>
                ...
                {ev.cuisine && <div style={s.cuisine}>{ev.cuisine}</div>}
                <div style={s.links}>
                  {ev.instagram && (
                    <a
                      href={instagramUrl(ev.instagram)}
                      target="_blank"
